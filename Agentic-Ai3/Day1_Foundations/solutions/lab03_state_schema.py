...
class ReconciliationState(TypedDict, total=False):
    ...
    security_blocked: bool          # NOT 'security_hold' - see Day 3 Lab 4
...
def make_initial_state(row: dict[str, str], run_id: str) -> ReconciliationState:
    ...
    nodes will believe.
    """
    # <<<BLANK hint="Return a ReconciliationState with run_id, txn_id, bank_customer_raw, bank_amount_usd (float), bank_reference, value_date, end_state 'OPEN', requires_human False, and empty trace/errors/security_flags lists">
    return ReconciliationState(
        run_id=run_id,
        txn_id=row["txn_id"],
        bank_customer_raw=row["customer_name_raw"].strip(),
        bank_amount_usd=float(row["amount_usd"]),
        bank_reference=row["reference_text"].strip(),
...
class TracedState(TypedDict, total=False):
    ...
    # <<<BLANK hint="Annotate `trace` as list[str] with operator.add so entries accumulate">
    trace: Annotated[list[str], operator.add]  # append - accumulates (correct)
    # >>>
