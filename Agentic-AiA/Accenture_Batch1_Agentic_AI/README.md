...
| Day 3 — Governance & Observability | **deck + guide built (design-locked); labs not built** |
| Capstone — Payment & Reconciliation | **deck + guide built (design-locked); build not started** |
| Gap remediation lab (HITL, gap G3) | not built — **blocks the capstone** |
...
```
00_Program/          requirements, .env template, verifier,
...
Day3_Governance/     deck · guide (design-locked) · labs not built
Capstone/            deck · guide (design-locked) · build not started
...
1,538 checks across 10 solutions, 10 starters and 20 notebooks: syntax, required
...
2. **Human-in-the-loop is the highest-priority open gap (G3).** The Capstone's
   `QUERY` state needs LangGraph `interrupt` + a checkpointer, and no Day 1–3 lab
   teaches it. Learners can route to `QUERY` but not resume from it.
