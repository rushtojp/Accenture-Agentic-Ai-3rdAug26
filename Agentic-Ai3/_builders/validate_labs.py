...
# Non-lab Python that must obey the same structural rules (S12 in particular).
EXTRA_MODULES = ["Capstone/src/pipeline.py", "Capstone/src/domain.py",
                 "Capstone/src/security.py", "Capstone/src/batch.py"]
...
def validate_solution(path: Path, r: Result) -> dict:
    ...
    # --- S12: node name must not collide with a state key -------------------
    # LangGraph keys nodes and state fields in the same namespace. Newer
    # versions raise ValueError; older versions accept it SILENTLY, so this
    # cannot be left to the installed version to catch.
    ...
    for call in ast.walk(tree):
        if (isinstance(call, ast.Call) and isinstance(call.func, ast.Attribute)
                and call.func.attr == "add_node" and call.args
                and isinstance(call.args[0], ast.Constant)
                and isinstance(call.args[0].value, str)):
            node_names.add(call.args[0].value)
    # node lists built as tuples then looped: ("name", fn)
    for tup in ast.walk(tree):
        if (isinstance(tup, ast.Tuple) and len(tup.elts) == 2
                ...
                and tup.elts[1].id.startswith("node_")):
            node_names.add(tup.elts[0].value)

    collisions = sorted(node_names & state_keys)
    r.check(not collisions, "S12",
            f"{name}: node name(s) collide with state key(s): {collisions}. "
            "Newer LangGraph raises; older versions fail silently. "
            "Convention: nodes are actions, state fields are facts.")
...
def main() -> int:
    ...
    # --- cross-cutting: the Capstone package obeys the same collision rule ---
    print("\nCapstone package")
    print("-" * 78)
    for rel in EXTRA_MODULES:
        ...
        try:
            ...
            r.check(False, "S1", f"{rel}: syntax error line {exc.lineno}")
            continue
        r.check(True, "S1", "")
        ...
        collisions = sorted(node_names & state_keys)
        r.check(not collisions, "S12",
                f"{rel}: node name(s) collide with state key(s): {collisions}")

        status = f"{GREEN}PASS{RESET}" if len(r.failures) == before else f"{RED}FAIL{RESET}"
        print(f"  {status}  {rel:<42} {len(node_names)} node(s), {len(state_keys)} state key(s)")
        for f in r.failures[before:]:
            print(f"        {RED}{f}{RESET}")
