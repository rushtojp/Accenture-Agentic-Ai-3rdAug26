...
def node_human_review(state: HitlState) -> dict:
    ...
    # <<<BLANK hint="Call interrupt() with a payload containing txn_id, variance_usd, matched_invoice, reason_evidence, and an 'ask' string plus the list of legal options. Store the returned value in `decision`.">
    decision = interrupt({
        "txn_id": state["txn_id"],
...
def build_graph(checkpointer):
    ...
    # <<<BLANK hint="Compile the builder with checkpointer=checkpointer — without it, interrupt() has nowhere to persist state and resume is impossible">
