...
sheet(
    ...
    [
        ...
        ["A11", "LangGraph node name vs state key namespace",
         "Nodes and state fields share one namespace. A node named 'security_hold' beside a state field of the same name raises ValueError on newer LangGraph and is accepted SILENTLY on older versions - so it works on the author's machine and fails on the delivery machine.",
         "HIGH",
         "FIXED: state key renamed to security_blocked (node stays security_hold). Runtime guards added in Capstone/src/pipeline.py and Day 3 Lab 4. Static check S12 in validate_labs.py scans every TypedDict key against every add_node literal, in labs AND the Capstone package - version-independent.",
         "None. Regression-tested by reintroducing the collision; S12 caught it."],
