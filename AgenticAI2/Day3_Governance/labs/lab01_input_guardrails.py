# ==========================================================================
# STARTER FILE - Day 3 Lab 1 - Input Guardrails (Anti-Prompt-Injection Shield)
# ==========================================================================
...
def normalise(text: str) -> str:
    ...
    # ------------------------------------------------------------------
    # TODO (Blank 1): NFKC-normalise `text`, translate away ZERO_WIDTH characters, collapse all runs of whitespace to single spaces, and lower-case the result
    # ------------------------------------------------------------------
...
def scan_input(text: str, *, source: str = "-") -> list[SecurityFinding]:
    ...
    # ------------------------------------------------------------------
    # TODO (Blank 2): For each (control, pattern, severity, detail) in RULES, search `subject` and append ONE SecurityFinding per matching control, recording match.start() as the position
    # ------------------------------------------------------------------
...
for label, text in [("clean", LEGITIMATE["BNK-1002"]), ("attack", ATTACKS["A1_system_override"])]:
    ...
    print(f"{label:<8} hold={result.get('security_hold', False)}  {result['trace'][0]}")
...
# - [ ] `node_input_guardrail` returns `security_flags` and `security_hold`.
#
