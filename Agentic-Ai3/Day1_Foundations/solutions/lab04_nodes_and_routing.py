...
def normalise_customer(raw: str) -> str:
    ...
    # <<<BLANK hint="Lower-case and strip `raw`, look it up in CUSTOMER_ALIASES, return the raw value unchanged if absent">
    key = raw.strip().lower()
    return CUSTOMER_ALIASES.get(key, raw.strip())
    # >>>
...
def rule_p4_customer_invoice(bank: dict, ar: list[OpenItem], remit: dict) -> MatchResult | None:  # noqa: ARG001
    ...
    if not ident["invoice_no"]:
        return None
    # <<<BLANK hint="Scan `ar` for an item whose customer_name equals bank['customer'] and invoice_no equals ident['invoice_no']; return MatchResult(4, '2-way', ...) or None">
    for item in ar:
        if item.customer_name == bank["customer"] and item.invoice_no == ident["invoice_no"]:
            return MatchResult(4, "2-way", item.invoice_no, item.amount_usd,
                               f"customer + invoice {ident['invoice_no']}")
    return None
    # >>>
...
def route_after_matching(state: dict) -> str:
    ...
    # <<<BLANK hint="Return 'matched' when state['matched_priority'] is truthy, otherwise 'exception'">
