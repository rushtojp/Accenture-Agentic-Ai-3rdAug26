...
def retrieve(query: str, txn_id: str | None = None,
    ...
    # <<<BLANK hint="Build a list of Evidence from the parallel result lists (ids, documents, distances, metadatas), keeping only rows whose distance <= threshold">
    kept: list[Evidence] = []
    for chunk_id, text, dist, meta in zip(
        ...
        if dist <= threshold:
            kept.append(Evidence(chunk_id, text, round(dist, 4),
                                 meta.get("txn_id", "?"), meta.get("customer", "?")))
    return kept
    # >>>
...
def node_remittance_search(state: dict) -> dict:
    ...
    with trace_node(log, "remittance_search", run_id, txn_id=txn_id) as out:
        ...
        # <<<BLANK hint="Build the retrieval query FROM STATE: a negative variance asks why money was withheld, a positive variance asks about the excess, and zero asks which invoices the payment covers">
        if variance < 0:
            query = "reason the customer withheld or deducted part of the payment"
        elif variance > 0:
            query = "explanation for the excess or duplicate amount paid"
        else:
            query = "invoice numbers and amounts covered by this payment"
        # >>>
