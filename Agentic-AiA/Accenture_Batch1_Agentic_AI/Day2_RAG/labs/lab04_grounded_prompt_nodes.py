# ==========================================================================
# STARTER FILE - Day 2 Lab 4 - Building Grounded Prompt Nodes
# ==========================================================================
...
class DeductionFinding(BaseModel):
    ...
    def check_grounded(self, source_text: str) -> None:
        ...
        """
        # ------------------------------------------------------------------
        # TODO (Blank 1): Exempt UNKNOWN (it makes no claim). Require non-empty evidence for any coded finding. Then normalise whitespace on BOTH the evidence and the source, lower-case both, and raise ValueError unless the evidence is a substring of the source
        # ------------------------------------------------------------------
...
def extract_deduction(evidence_text: str, *, run_id: str = "-",
    ...
    for attempt in (1, 2):
        try:
            ...
            # ------------------------------------------------------------------
            # TODO (Blank 2): Validate `payload` into a DeductionFinding, then call finding.check_grounded(evidence_text), and return (finding, problems)
            # ------------------------------------------------------------------
