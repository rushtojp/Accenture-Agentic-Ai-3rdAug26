...
def redact(text: str) -> tuple[str, list[dict]]:
    ...
    for rule in RULES:
        # <<<BLANK hint="Find every match of rule.pattern in `clean`. For each, build the replacement: if rule.keep_tail is 0 use f'[REDACTED:{rule.name}]', otherwise mask all but the last keep_tail characters with '*'. Replace in `clean` and append a dict with rule, cls and count to `inventory`.">
        ...
        for m in matches:
            ...
            if rule.keep_tail == 0:
                ...
                tail = found[-rule.keep_tail:]
                replacement = "*" * max(0, len(found) - rule.keep_tail) + tail
...
def to_envelope(exc: Exception, code: str = "INTERNAL") -> ErrorEnvelope:
    ...
    # <<<BLANK hint="Redact the exception text BEFORE it reaches the internal log - log aggregators get breached too. Use redact() on f'{type(exc).__name__}: {exc}'">
    detail, _ = redact(f"{type(exc).__name__}: {exc}")   # redact even internally
    # >>>
