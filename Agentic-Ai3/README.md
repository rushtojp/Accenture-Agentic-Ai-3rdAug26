...
2,497 checks across 16 solutions, 16 starters, 32 notebooks and the Capstone package: syntax, required
...
- **A node is an action; a state field is a fact.** LangGraph keys nodes and
  state fields in one namespace, so `security_hold` (node) pairs with
  `security_blocked` (field), never the same name twice. Check **S12** in
  `validate_labs.py` enforces this statically across the labs *and* the Capstone
  package, because newer LangGraph raises on a collision while older versions
  accept it silently.
